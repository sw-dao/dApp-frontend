...
describe("Quote Endpoint", () => {
  ...
  it("should allow selected token addresses", async () => {
    ...
    const call = `/api/quotes/${SWYF}/${ETH}/1000000000000000000?chainId=0x1`;
    const quote = {
      price: 10,
      sellAmount: "1000000000000000000",
      buyTokenPercentageFee: 0.25,
      ...
      .onGet(
        ...
        expect.objectContaining({
          params: {
            ...
            feeRecipient: "0x52dddf6a08d2787f2629d582921a684a9e4d2e31",
            sellToken: ETH,
            takerAddress: "0x52dddf6a08d2787f2629d582921a684a9e4d2e31",
          },
        })
      )
      .reply(200, quote);
    ...
    const { body, status } = await request(server)
      .post(call)
      .send({
        data: { account: "0x52dddf6a08d2787f2629d582921a684a9e4d2e31" },
        chainId: "0x1",
      });

    // then
    expect(status).toEqual(200);
    expect(body).toMatchObject<QuoteResponse>({ price: +quote.price });
    expect(mock.history.get.length).toBe(1);
  });

  it("should return a error on non-permitted token swap", async () => {
    // given
    const ETH = SwappableTokens.ERC20.ETH;
    const USDC = SwappableTokens.ERC20.USDC;
    const call = `/api/quotes/${ETH}/${USDC}/1000000000000000000`;
    ...
    const res = await request(server)
      .post(call)
      .send({ account: "0x52dddf6a08d2787f2629d582921a684a9e4d2e31" });
  ...
  it("should return a 400 on non-address input", async () => {
    // given
    const call = "/api/quotes/notAnAddres/012345689/1000000000000000000";
    mock.onAny().abortRequest();
    const server = initServer();
  ...
  it("should return a 400 on non-permitted token address", async () => {
    ...
    const USDC = SwappableTokens.ERC20.USDC;
    const call = `/api/quotes/${invalidAddress}/${USDC}/1000000000000000000`;
    mock.onAny().abortRequest();
    const server = initServer();
  ...
  it("should return a 400 on non-valid wei amount", async () => {
    // given
    const call = "/api/quotes/notAnAddres/012345689/10.0000000000000000";
    mock.onAny().abortRequest();
    const server = initServer();
