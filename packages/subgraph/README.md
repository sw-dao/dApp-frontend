...
First run `yarn prepare:[NETWORK]` to create the appropriate `subgraph.yaml`

See the files in `/config` for network specifi configs

> Note on Ropsten: to fake some transaction flow we use USDT and DAI tokens instead of specific TokenSets. For simplicity.
